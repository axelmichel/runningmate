...
def mock_file_data():
    """Mock TCX DataFrame"""
    data = {
        "Time": pd.date_range(start="2025-02-15 10:00:00", periods=10, freq="min").astype(str),
...
def test_compute_data(mock_tcx_data):
    ...
    assert computed["elevation_gain"] == 50, "Elevation gain should be calculated correctly."
    assert computed["avg_speed"] > 0, "Average speed should be positive."
    assert computed["avg_heart_rate"] == expected_heart_rate, f"Expected {expected_heart_rate}, but got {computed['avg_heart_rate']}"
    assert computed["avg_power"] == expected_power, f"Expected {expected_power}, but got {computed['avg_power']}"
...
@patch("importer.file.tcx_file.parse_tcx", return_value=(mock_file_data(), "Running"))  # ✅ Fixed mock
@patch("importer.file.tcx_file.TcxFileImporter.compute_data", return_value={
    "date": 1739296446,
    "distance": 5.0,
    "elevation_gain": 100,
    "duration": 1200,
...
@patch("importer.file.tcx_file.TcxFileImporter.plot_stats")
...
def test_archive_file(tcx_importer):
    ...
    with tarfile.open(archive_path, "r:gz") as tar:
        assert "test.tcx" in tar.getnames(), "Archive should contain the original TCX file."


def test_generate_activity_title():
    """Test generate_activity_title() to ensure correct time-of-day labels."""
    morning = generate_activity_title(ViewMode.RUN, pd.Timestamp("2025-02-15 07:00:00", tz="UTC").timestamp())
    afternoon = generate_activity_title(ViewMode.RUN, pd.Timestamp("2025-02-15 14:00:00", tz="UTC").timestamp())
    evening = generate_activity_title(ViewMode.RUN, pd.Timestamp("2025-02-15 18:30:00", tz="UTC").timestamp())
    night = generate_activity_title(ViewMode.RUN, pd.Timestamp("2025-02-15 23:00:00", tz="UTC").timestamp())
