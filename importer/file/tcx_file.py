...
from processing.system_settings import mapActivityTypes, ViewMode
...
class TcxFileImporter:
    ...
    def upload(self):
        file_path, _ = QFileDialog.getOpenFileName(None, "Select TCX File", "", "TCX Files (*.tcx)")
    ...
    def process_file(self, file_path):
        ...
        computed_data["title"] = generate_activity_title(type, computed_data["date"])
        ...
        self.plot_stats(name, df)

    def process_run(self, df, computed_data):
        avg_steps, total_steps = calculate_steps(df)
        computed_data["avg_steps"] = int(round(avg_steps, 0)) if not np.isnan(avg_steps) else 0
        computed_data["total_steps"] = int(total_steps) if not np.isnan(total_steps) else 0
    ...
    def archive_file(self, file_path):
        ...
        tar_gz_filename = os.path.join(self.file_path, f"{base_name}.tar.gz")  # Output tar.gz file

        with tarfile.open(tar_gz_filename, "w:gz") as tar:
            tar.add(file_path, arcname=base_name)  # Store file inside archive without full path

    @staticmethod
    def compute_data(df):
        total_distance = df["Distance"].iloc[-1] if "Distance" in df.columns else 0
        total_time = (
            (pd.to_datetime(df["Time"].iloc[-1]) - pd.to_datetime(df["Time"].iloc[0])).total_seconds()
            if "Time" in df.columns else 0
        )
