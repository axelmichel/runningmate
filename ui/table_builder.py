...
from processing.system_settings import ViewMode, SortOrder
...
HEADERS = {
    ...
    ViewMode.RUN: ["date_time", "title", "duration", "distance", "elevation_gain", "avg_speed", "avg_pace",
                   "avg_heart_rate", "total_steps", "pause"],
    ViewMode.WALK: ["date_time", "title", "duration", "distance", "elevation_gain", "avg_speed", "avg_pace",
                    "avg_heart_rate", "total_steps"],
    ViewMode.CYCLE: ["date_time", "title", "duration", "distance", "elevation_gain", "avg_speed", "avg_pace",
                     "avg_heart_rate", "avg_power", "pause"],
}
...
class TableBuilder:
    ...
    @staticmethod
    def setup_table(table_widget: QTableWidget, activity_type: ViewMode, data: list, parent=None):
        if not data:
        ...
        for row_index, row_data in enumerate(data):
            ...
            id_item = QTableWidgetItem(str(row_data.get("activity_id", row_index)))  # Use row index if no ID
            id_item.setFlags(Qt.ItemFlag.ItemIsEnabled)  # Prevent editing
            table_widget.setItem(row_index, len(headers), id_item)  # Store ID in last column
        ...
        if parent:
            table_widget.cellClicked.connect(lambda row, col: TableBuilder.handle_row_click(table_widget, row, parent))
            table_widget.horizontalHeader().sectionClicked.connect(
                lambda index: parent.sort_by_column(activity_type, column=headers[index])
            )

    @staticmethod
    def update_header_styles(table_widget: QTableWidget, activity_type: ViewMode, column, sort_order: SortOrder):
        """
        ...
        order = Qt.SortOrder.AscendingOrder if sort_order == SortOrder.ASC else Qt.SortOrder.DescendingOrder
    ...
    def handle_row_click(table_widget, row, parent):
        ...
        id_item = table_widget.item(row, table_widget.columnCount() - 1)  # ✅ Get the hidden ID column
        if id_item:
            row_id = id_item.text()  # ✅ Extract the stored ID
            correct_row_data = next((data for data in table_widget.row_data if str(data.get("activity_id")) == row_id),
                                    None)
