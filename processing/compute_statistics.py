...
def compute_run_statistics(
    ...
    data = {
        ...
        "Average Pace (min/km)": format_hour_minute(
            avg_pace
        ),  # Convert to MM:SS format
        "Fastest Kilometer (min/km)": format_hour_minute(
            fastest_pace
        ),  # Convert to MM:SS format
        "Slowest Kilometer (min/km)": format_hour_minute(
            slowest_pace
        ),  # Convert to MM:SS format
...
def compute_run_db_data(
    ...
    avg_speed = (
        total_distance / (total_time.total_seconds() / 3600)
    ...
    formatted_data = (
        ...
        month,
        (
            pd.to_datetime(df["Time"].iloc[0]).strftime("%H:%M")
            if "Time" in df.columns
            else "00:00"
        ),
