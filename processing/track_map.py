...
class TrackMap:
    ...
    def _create_track_map(self):
        ...
        folium.PolyLine(route, color=THEME.MAIN_COLOR, weight=4, opacity=0.7).add_to(
            activity_map
        )
        ...
        segment_distance = (
            1 if sport_type in [ViewMode.RUN, ViewMode.WALK] else 5
        )  # 1K for running/walking, 5K for cycling
    ...
    def _create_heatmap(self, map_type: str):
        ...
        if map_type == "heart_rate":
            ...
            color_scale = LinearColormap(
                reversed_colors, vmin=min_value, vmax=max_value
            )
        else:
        ...
        for i in range(len(df) - 1):
            ...
            folium.PolyLine(coords, color=color, weight=5, opacity=0.8).add_to(
                activity_map
            )
        ...
        activity_map.save(html_path)
        return self.activityData.save_activity_map(
            self.activity_id, map_type, html_path
        )
